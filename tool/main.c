...
static int first_fit(nv_atlas_positions* positions, width_height* images_data,
                     int image_data_count, int max_size) {
  ...
  for (int i = 0; i < image_data_count; i++) {
    ...
    // fit it inside
    if (data.width > max_size || data.height > max_size) return -1;
    if (pos_x + data.width >= max_size) {
      pos_x = 0;
      pos_y += row_height;
      row_height = 0;
      if (pos_y + data.height >= max_size) return -1;
    }
    if (row_height < data.height) row_height = data.height;
    positions[i].id = i;
    positions[i].x = pos_x;
    positions[i].y = pos_y;
...
int main(int argc, char* argv[]) {
  ...
  for (int i = 0; i < file_list_count; i++) {
    ...
    unsigned char* open_image =
        stbi_load(file_list_paths[index], &width, &height, NULL, 4);
    for (int x = 0; x < width; x++) {
      for (int y = 0; y < height; y++) {
        unsigned int lil_pos = (x * 4) + (y * width * 4);
        unsigned int pos =
            ((positions[i].x + x) * 4) + ((positions[i].y + y) * 4 * size);
